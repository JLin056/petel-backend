...
public class BOOK007SvcImpl implements BOOK007Svc {
    ...
    public Res<BOOK007Tranrs> book007(Req<BOOK007Tranrq> requestBody) throws Exception {
        ...
        if (ordersEntity.getPaymentId() != 1) {
            log.error("[BOOK-007] 您的付款方式不適用這隻 API");
            throw new InvalidPaymentMethodException();
        }

        log.info("[BOOK-007] 訂單編號為 {} 的訂單，組合付款參數成功", orderId);
        return new Res<>(new ResMwHeader(ReturnCodeAndDescEnum.SUCCESS), new BOOK007Tranrs(MERCHANT_ID, new BOOK007TranrsRqHeader(ZonedDateTime.now(ZoneId.of("Asia/Taipei")).toEpochSecond()), CodeUtil.dataEncrypt(requestBody.getTranrq().getData(), HASH_KEY, HASH_IV)));
    }
}
