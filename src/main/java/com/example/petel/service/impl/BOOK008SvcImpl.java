...
public class BOOK008SvcImpl implements BOOK008Svc {
    ...
    @Override
    public Res<BOOK008Tranrs> book008(Req<BOOK008Tranrq> requestBody) throws Exception {

        long orderId = requestBody.getTranrq().getOrderId();

        OrdersEntity ordersEntity = ordersRepository.findById(orderId).orElseThrow(() -> {
            log.error("[BOOK-008] 查無訂單編號為 {} 的資料，無法組合付款參數", orderId);
            return new DataNotFoundException();
        });

        if (ordersEntity.getPaymentId() != 2) {
            log.error("[BOOK-008] 您的付款方式不適用這隻 API");
        ...
        log.info("[BOOK-008] 訂單編號為 {} 的訂單，組合付款參數成功", orderId);
